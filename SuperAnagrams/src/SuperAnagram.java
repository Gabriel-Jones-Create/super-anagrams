public class SuperAnagram {
	...
	int[] scoreboard = new int[26]; // creates an integer array with 26 positions to represent every letter of the
									// alphabet
	...
	/**
	 * Returns boolean value based on whether or not the first input is a super
	 * Anagram of the second input
	 * 
	 ...
	 * @return <code>true</code> if the first input is a super anagram of the
	 *         second; <code>false</code> otherwise
	 */
	public boolean isSuperAnagram(String initMatch, String initInitial) {
		// creates a character array out of the input string and sets it to the initial
		// array
		initialArray = initInitial.toCharArray();
		// creates a character array out of the input string and sets it to the match
		// array
		matchArray = initMatch.toCharArray();
		// creates a working character array and sets it to the respective other
		// character array with only lower case letters
		...
		// for every character in the workingInitArray the index of the ASCII code of
		// the letter is added to the score board array
		for (int i = 0; i < workingInitArr.length; i++) {
			scoreboard[workingInitArr[i] - 97]++;
		}
		// for every character in the workingMatchArr the index of the ASCII code of the
		// letter is subtracted from the score board array
		for (int i = 0; i < workingMatchArr.length; i++) {
			scoreboard[workingMatchArr[i] - 97]--;
		}
		// for every value in the score board array, if the number is less than 0 then
		// the the input is not a super anagram, otherwise, it is.
	 ...
	 * 
	 * @param ch character array that may have spaces, numbers, or special
	 *           characters
	...
	private char[] simplify(char[] ch) {
		...
		int iter = 0;
		// for every value in the ch array, if the value is a letter then it adds that
		// to the simplified character array, otherwise it does nothing
